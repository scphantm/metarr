...
## Note on this repo's design docs

Metarr also has `documentation/modules/design/` — the source of truth for system design (workflow engine and beyond),
separate from `CONTEXT.md`/ADRs. Per the root `CLAUDE.md`: check it before implementing changes in an area it covers,
and never edit it without the user's explicit go-ahead, even to fix what looks like a stale passage.
