...
To aid with debugging, you can use Handlebars`' built-in `log` helper to print information from the UI model to the console.
The helper accepts a single value or variable and will format it in the same way as the `console.log` function in Node.js.

Let's use the `log` helper to print the URL of each page in the site.
We'll add this logic to the [.path]_footer-content.hbs_ partial.
Since the template logic is going to run on every page, we add a conditional so that it only runs on the site start page.
We know it's the site start page because the value of `page.url` will match the value of `site.homeUrl`.
...
Here we are leveraging the dynamic `files` property on each component verison introduced in Antora 3.2.0.
If you are using an earlier version of Antora, you will need to create a helper that returns the files for a component version.
To ensure we're only logging pages, we check whether the family is `page`.

This snippet prints the URL of each page.
If you want to print the whole object, use the following statement instead:
...
You could use a helper to simplify the logic.
In fact, a helper can print to the console itself using `console.log`.
