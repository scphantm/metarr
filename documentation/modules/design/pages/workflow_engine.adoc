...
The graph is a generated proto message, `WorkflowGraph`, like every other model that crosses a language boundary (see `docs/adr/0005`). Go and TypeScript both read the generated type; `internal/shared/workflow` aliases it (`Graph = metarrv1.WorkflowGraph`), and it is the shape the workflow service persists.
...
message WorkflowGraphNode {
  string id = 1;
  string type = 2;               // free string — the catalog owns the vocabulary
  string catalog_id = 3;
  WorkflowGraphPosition position = 4;
  google.protobuf.Struct settings = 5;   // structured, not a typed per-type union
  repeated string promoted = 6;
  string label = 7;
  google.protobuf.Struct extra = 8;      // whatever this build's schema does not model
}
----

`kind` on an edge is the `WorkflowEdgeKind` enum (`control` / `data`) — a closed vocabulary the engine owns. A node's `type` stays a free string so a new node type is a catalog data change, never a proto change or a release.

The UI keeps an adapter (`graphAdapter.ts`) between this message and React Flow's own node/edge shape — React Flow's `type` field means *which component renders this*, a presentation concern distinct from node identity — and it is where the editor's per-instance colour overrides are moved into and out of `extra`, since the message has no field for them.

> **A node this build does not recognise survives being loaded and saved.** The graph carries its open content as structured values: a node's `settings` and its `extra` field are both `google.protobuf.Struct`, so a node whose `type` this build does not know, and settings it does not know, round-trip through storage unchanged. This is no longer an accident of opaque `[]bson.M` storage — it is an asserted behaviour, guarded by a store-and-load round-trip test at the workflow service seam (`internal/server/services/workflows_test.go`). A typed per-node-type settings union was considered and rejected for exactly this reason: it is the shape that silently drops user work. See §9.
...
- **Tombstone rendering.** A `missing`/`incompatible` node renders as a distinct tombstone that **preserves its stored settings and edges verbatim**. The flow opens, displays, and re-saves without loss; it simply cannot run. The generated `WorkflowGraph` message makes this deliberate rather than incidental — `type` is a free string and `settings`/`extra` are structured values — and the guarantee is asserted by the round-trip test named in "Workflow document" above.
