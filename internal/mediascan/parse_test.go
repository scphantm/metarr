...
func TestParseImageName(t *testing.T) {
	tests := []struct {
		...
		{baseName: "poster", wantType: ImagePoster, wantRecognized: true},
		{baseName: "folder", wantType: ImagePoster, wantRecognized: true},
		{baseName: "cover", wantType: ImagePoster, wantRecognized: true},
		{baseName: "fanart", wantType: ImageBackdrop, wantRecognized: true},
		{baseName: "background", wantType: ImageBackdrop, wantRecognized: true},
		{baseName: "backdrop-1", wantType: ImageBackdrop, wantIndex: 1, wantRecognized: true},
		{baseName: "backdrop2", wantType: ImageBackdrop, wantIndex: 2, wantRecognized: true},
		{baseName: "banner", wantType: ImageBanner, wantRecognized: true},
		{baseName: "clearlogo", wantType: ImageLogo, wantRecognized: true},
		{baseName: "landscape", wantType: ImageThumb, wantRecognized: true},
		{baseName: "discart", wantType: ImageDisc, wantRecognized: true},
		{baseName: "clearart", wantType: ImageClearArt, wantRecognized: true},
		// Plex keeps season artwork at the series root.
		{baseName: "Season01", wantType: ImagePoster, wantSeason: intPtr(1), wantRecognized: true},
		{baseName: "season01-poster", wantType: ImagePoster, wantSeason: intPtr(1), wantRecognized: true},
		{baseName: "season-specials-poster", wantType: ImagePoster, wantSeason: intPtr(0), wantRecognized: true},
		{baseName: "The Matrix", wantRecognized: false},
		{baseName: "random-file", wantRecognized: false},
	}

	for _, test := range tests {
		t.Run(test.baseName, func(t *testing.T) {
			got, recognized := parseImageName(test.baseName)
			if recognized != test.wantRecognized {
				t.Fatalf("recognized = %v, want %v", recognized, test.wantRecognized)
			}
			if !recognized {
				return
			}
			if got.ImageType != test.wantType {
				t.Errorf("ImageType = %q, want %q", got.ImageType, test.wantType)
			}
			if got.Index != test.wantIndex {
				t.Errorf("Index = %d, want %d", got.Index, test.wantIndex)
			}
			if !reflect.DeepEqual(got.SeasonNumber, test.wantSeason) {
				t.Errorf("SeasonNumber = %v, want %v", derefSeason(got.SeasonNumber), derefSeason(test.wantSeason))
			}
...
func TestParseSidecarSuffix(t *testing.T) {
	tests := []struct {
		suffix string
		want   SubtitleAttributes
	}{
		{".en", SubtitleAttributes{Language: "en"}},
		{".eng", SubtitleAttributes{Language: "eng"}},
		{".en.forced", SubtitleAttributes{Language: "en", Forced: true}},
		{".en.foreign", SubtitleAttributes{Language: "en", Forced: true}},
		{".en.sdh", SubtitleAttributes{Language: "en", HearingImpaired: true}},
		{".en.cc", SubtitleAttributes{Language: "en", HearingImpaired: true}},
		{".default.en.forced", SubtitleAttributes{Language: "en", Default: true, Forced: true}},
		{".Director Commentary.en", SubtitleAttributes{Language: "en", Title: "Director Commentary"}},
		{"", SubtitleAttributes{}},
	}

	for _, test := range tests {
		t.Run(test.suffix, func(t *testing.T) {
			got := parseSidecarSuffix(test.suffix)
			if !reflect.DeepEqual(got, test.want) {
				t.Errorf("parseSidecarSuffix(%q)\n got: %+v\nwant: %+v", test.suffix, got, test.want)
			}
		})
	}
}
