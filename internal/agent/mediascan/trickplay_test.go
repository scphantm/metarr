...
func assertTile(t *testing.T, sidecar *scanmodel.SidecarFile, wantWidth, wantTileWidth, wantTileHeight, wantIndex int32) {
	...
	if sidecar.Type != scanmodel.SidecarTrickplay {
		t.Errorf("%s: Type = %q, want %q", sidecar.FileName, sidecar.Type, scanmodel.SidecarTrickplay)
	}
	if sidecar.Category != scanmodel.SidecarCategoryTrickplay {
		t.Errorf("%s: Category = %q, want %q", sidecar.FileName, sidecar.Category, scanmodel.SidecarCategoryTrickplay)
	}
...
func TestTrickplayInSeasonFolder(t *testing.T) {
	...
	result := scanTree(t, "The Show", scanmodel.TypeTV, files)
...
func TestTrickplayTilesAreNotImages(t *testing.T) {
	...
	images := sidecarsInCategory(mediaFile.Sidecars, scanmodel.SidecarCategoryImage)
	...
	if got := len(sidecarsInCategory(mediaFile.Sidecars, scanmodel.SidecarCategoryTrickplay)); got != 5 {
...
func TestTrickplayLooseFileHasNoResolution(t *testing.T) {
	result := scanTree(t, "The Movie (2019)", scanmodel.TypeMovie, map[string]string{
		"The Movie (2019).mkv":                 "video",
