...
func TestParseDirectoryType(t *testing.T) {
	for _, valid := range []string{"movie", "tv", "music_video"} {
		if _, err := scanmodel.ParseDirectoryType(valid); err != nil {
			t.Errorf("scanmodel.ParseDirectoryType(%q) error = %v", valid, err)
		}
	}
	for _, invalid := range []string{"", "anime", "Movie", "tvshow"} {
		if _, err := scanmodel.ParseDirectoryType(invalid); err == nil {
			t.Errorf("scanmodel.ParseDirectoryType(%q) succeeded, want an error", invalid)
		}
	}
}
