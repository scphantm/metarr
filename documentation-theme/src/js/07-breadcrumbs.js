(function () {
  ...
  function truncate(toolbar, breadcrumbs, truncationLength) {
    ...
    while (
      breadcrumbs.length &&
      toolbar.scrollHeight > toolbar.getBoundingClientRect().height
    ) {
      var breadcrumb = breadcrumbs.pop();
      var text = (breadcrumb =
        breadcrumb.querySelector("a") || breadcrumb).innerText.trim();
  ...
  function onResize(toolbar, breadcrumbs, truncationLength) {
    breadcrumbs.forEach(function (breadcrumb) {
      if (
        !(breadcrumb =
          breadcrumb.querySelector("a") || breadcrumb).hasAttribute("title")
      )
        return;
  ...
  (function () {
    ...
    var breadcrumbs = Array.prototype.slice.call(
      toolbar.querySelectorAll(".breadcrumbs li") || [],
    );
    if (breadcrumbs.length < 3) return;
    var truncationLength = parseInt(
      toolbar.querySelector(".breadcrumbs").dataset.truncationLength || 15,
      10,
    );
    ...
    window.addEventListener(
      "resize",
      onResize.bind(null, toolbar, breadcrumbs, truncationLength),
    );
  })();
})();
