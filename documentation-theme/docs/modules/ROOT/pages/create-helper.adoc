...
A helper must be defined as a (CommonJS module) JavaScript file in the [.path]_helpers_ directory of the UI bundle.
...
A UI helper is a CommonJS module.
That means it has access to the global `module` object.
The id and filename properties of that `module` object is basename of the helper relative to the pseudo `ui-helpers` subdirectory in the playbook directory.
The `require` function operates from that folder.
That means you can require packages that are dependencies of the playbook project.
The parent property of the module is the `@antora/page-composer` module.
That means you can use `module.parent.require` to require packages from the Antora installation (e.g., `./build-ui-model` or `@antora/logger`).
