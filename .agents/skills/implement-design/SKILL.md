...
The issue being passed will be the design issue.  Assign that issue to the current PR so its closed when the PR is closed.

The issue will have sub issues under it.  loop thru each of those sub issues one at a time running the `/implement` skill in a new clear session.
